...
from engine.defaults import TRAIN_PHASE, VAL_PHASE
...
class PoseTrack_Subset(VideoDataset):
    """
        PoseTrack
    """

    def __init__(self, cfg, phase, **kwargs):
        super(PoseTrack_Subset, self).__init__(cfg, phase, **kwargs)
        ...
        self.coco = COCO(
            osp.join(self.json_dir, 'posetrack_train.json' if self.is_train else 'DCPose828_MDCPose_0.01.json'))
        # osp.join(self.json_dir, 'posetrack_train.json' if self.is_train else 'DCPose828_MDCPose_0.005.json'))
    ...
    def _get_spatiotemporal_window(self, data_item,item_index):
        ...
        farthest_distance = self.distance if self.is_train else 2

        if not is_posetrack18:
            # posetrack17 current_idx 从1算起
            prev_delta_range = range(1, min(current_idx, farthest_distance))
            next_delta_range = range(1, min(num_frames - current_idx + 1, farthest_distance))
        else:
            # posetrack18 current_idx 从0开始算起
            prev_delta_range = range(1, min(current_idx + 1, farthest_distance))
            next_delta_range = range(1, min(num_frames - current_idx, farthest_distance))
        ### setting deltas
        ...
        trans = get_affine_transform(center, scale, r, self.image_size)  # 计算目标
        ...
        meta = {
            ...
            'dataset_sample_id': item_index,
            'person_id': data_item['person_id'],
            'bbox': data_item['box'],  # xywh
            # 'original_joints': data_item['joints_3d'],
            # 'original_joints_vis': data_item['joints_3d_vis'],
            # 'box': data_item['box']
        }
    ...
    def evaluate(self, cfg, preds, output_dir, boxes, img_path, *args, **kwargs):
        ...
        # save json for evaluate
        person_ids = kwargs.get('person_ids')
        person_ids = person_ids.astype(np.int32)
        all_initial_boxes = kwargs.get('all_initial_boxes')
        all_gt_keypoints = kwargs.get('all_gt_keypoints')
        image_path_list = kwargs.get('image_path')
        instance_headSize = 0.6 * np.linalg.norm(all_initial_boxes[:, 2:], axis=1, keepdims=True)
        gt_joints = all_gt_keypoints[:, :, :2]
        preds_joints = preds[:, :, :2]
        # gt 与 pred 的 距离 [0,1]
        dist = np.linalg.norm(np.subtract(gt_joints, preds_joints), axis=2) / (instance_headSize + 1e-7)
        keypoint_vis = all_gt_keypoints[:, :, 2]
        json_data = {
            "dist": dist.tolist(),
            "keypoint_vis": keypoint_vis.tolist(),
            'image_list': image_path_list,
            'person_id_list': person_ids.tolist(),
            'bbox': all_initial_boxes.tolist(),
            'gt_joints': all_gt_keypoints.tolist(),
            'pred_joints': preds.tolist()
        }
        write_json_to_file(json_data, osp.join(osp.abspath(output_dir + "/.."), f'{self.phase}_dist_statistics.json'))

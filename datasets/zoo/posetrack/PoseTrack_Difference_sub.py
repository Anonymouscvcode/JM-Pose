...
# np.warnings.filterwarnings('ignore', category=np.DeprecationWarning)


# np.warnings.filterwarnings('ignore', category=np.DeprecationWarning)

@DATASET_REGISTRY.register()
class PoseTrack_Difference_sub(VideoDataset):
    """
        PoseTrack_Temporal_Difference_Modeling
        Similar to PoseTrackAlignment
    """

    def __init__(self, cfg, phase, **kwargs):
        super(PoseTrack_Difference_sub, self).__init__(cfg, phase, **kwargs)
    ...
    def _get_spatiotemporal_window(self, data_item):
        ...
        next_delta_range = range(1, min((num_frames - current_idx) if is_posetrack18 else (
                num_frames - current_idx + 1), farthest_distance))
        # prev_delta_range=[1,2]
        # next_delta_range =[1,2]
        prev_delta_range = list(prev_delta_range)
        next_delta_range = list(next_delta_range)
        ...
        for index in self.image_set_index:
            ...
            for obj in objs:
                ...
                if self.clustering:
                    ...
                    for index, joint in enumerate(joints):
                        x, y, _ = joint
                        if joints_3d_vis[index, 0] == joints_3d_vis[index, 1] == 1:
                            joint[2] = 1
                        if x <= 0 or y <= 0 or x > self.image_size[0] or y > self.image_size[1]:
                            joints[index] = [0, 0, 0]

                    clustering_joints.append(np.expand_dims(joints[:, :], 0))
